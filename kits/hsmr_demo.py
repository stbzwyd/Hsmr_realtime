...
#初始det_bs=10，det_mis=512， rec_bs=300，max_instances=5
def parse_args():
    ...
    parser.add_argument('-t', '--input_type', type=str, default='auto', help='Specify the input type. auto: file~video, folder~imgs,camera: webcam, stream: video stream', choices=['auto', 'video', 'imgs','camera', 'stream'])
    ...
    parser.add_argument('--det_bs', type=int, default=1, help='The max batch size for detector.')
    parser.add_argument('--det_mis', type=int, default=256, help='The max image size for detector.')
    parser.add_argument('--rec_bs', type=int, default=50, help='The batch size for recovery.')
...
def load_inputs(args, MAX_IMG_W=512, MAX_IMG_H=512):
    # 1. Inference inputs type.
    inputs_path_str = args.input_path  # 保持原始字符串
    inputs_type = args.input_type

    # 自动推断输入类型 wyd
    if inputs_type == 'auto':
        if inputs_path_str.isdigit():  # 摄像头 ID
            inputs_type = 'camera'
        elif Path(inputs_path_str).is_file():  # 视频文件
            inputs_type = 'video'
        elif Path(inputs_path_str).is_dir():  # 图像目录
            inputs_type = 'imgs'
        else:  # 假设是流 URL
            inputs_type = 'stream'
    ...
    get_logger(brief=True).info(f'🚚 Loading inputs from: {inputs_path_str}, regarded as <{inputs_type}>.')

    # 2. Load inputs.
    if inputs_type in ['video', 'imgs']:
        #原有逻辑
        inputs_meta = {'type': inputs_type}
        inputs_path = Path(inputs_path_str)  # 转换为 Path 对象
        ...
        return raw_imgs, inputs_meta #这一行是加的
    elif inputs_type in ['camera', 'stream']:
        # 新逻辑：处理摄像头或视频流
        inputs_meta = {'type': inputs_type, 'seq_name': f'{inputs_type}_{inputs_path_str}'}
    try:
        # 初始化视频捕获
        if inputs_type == 'camera':
            cap = cv2.VideoCapture(int(inputs_path_str))  # 摄像头 ID
        else:  # stream
            cap = cv2.VideoCapture(inputs_path_str)  # 流 URL 或视频文件路径
        if not cap.isOpened():
            raise ValueError(f"Failed to open {inputs_type} source: {inputs_path_str}")
        def frame_generator():
            while cap.isOpened():
                try:
                    while cap.isOpened():
        ...
        return frame_generator(), inputs_meta
    except Exception as e:
            get_logger(brief=True).error(f"Error initializing {inputs_type} source: {e}")
            raise
    ...
    #get_logger(brief=True).info(f'📦 Totally {len(raw_imgs)} images are loaded.') #放上面了

    # return raw_imgs, inputs_meta #移到上面了
